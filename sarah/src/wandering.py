...
#	self.joint_state_received = False
	...
	#### front wall
	# Getting angles 5 degrees left and right of 0
	a =msg.ranges[0:5]
	a+= msg.ranges[355:360]
	...
	for i in a:
		if not math.isinf(i) and not math.isnan(i):
			...
			count-=1;
	#print('Front total: {0}, count: {1:2.2f}'.format(total, count))		
	if count != 0:
		frontwall =total/count;
	else:
		#All lidars are infinite, there is nothing for many meters.
		frontwall = float('inf')
...
#		print(GetStateValidity())
		if frontwall <= self.detectionRange:
			# Obstacle in front of you, stop.
